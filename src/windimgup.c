...
HWND
createLabel(HWND parent, UINT_PTR controlId, const char *label, HFONT font, int posX, int posY, int width, int height) {
...
HWND createCheckbox(HWND parent, UINT_PTR controlId, int posX, int posY, int width, int height) {
    HWND checkbox = CreateWindowExA(0, "BUTTON", NULL, WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX | BS_MULTILINE, posX,
                                    posY, width, height, parent, (HMENU) controlId, hInst, 0);
...
HWND createProgress(HWND parent, UINT_PTR controlId, int max, int posX, int posY, int width, int height) {
    HWND progressBarHwnd = CreateWindowEx(0, PROGRESS_CLASS, NULL, WS_CHILD | WS_VISIBLE | PBS_SMOOTH, posX, posY,
                                          width, height, parent, (HMENU) controlId, hInst, NULL);
...
DWORD __stdcall uploadHelper(uploadParam *param) {
    ...
    switch ((errCode = uploadWebhook(webhook_start, dataLocked, (DWORD) dataSize, param->filename, &url,
                                     updateProgress))) {
    ...
    *param->filename = 0;
    ...
    switch (getPngFromClipboard(&param->data)) {
        case 1:
            ...
            goto rip;
        case 2:
            MessageBoxA(NULL, "Failed to initialize the PNG encoder", "Error", MB_ICONERROR);
            goto rip;
        case 3:
            MessageBoxA(NULL, "Failed to open clipboard", "Error", MB_ICONERROR);
            goto rip;
        case 4:
            MessageBoxA(NULL, "No image in clipboard", "Error", MB_ICONWARNING);
            goto rip;
        case 5:
        case 6:
            MessageBoxA(NULL, "Failed to allocate the in-memory stream", "Error", MB_ICONERROR);
            goto rip;
    }
